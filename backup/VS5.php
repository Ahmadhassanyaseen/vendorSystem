...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["method"]) && $_POST["method"] == "CreateVendor") {
        ...
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
    } else if (isset($_POST["method"]) && $_POST["method"] == "fetchAllVendors") {
        $result = fetchAllVendors($_POST['vndid']);
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
        ...
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
    } else if (isset($_POST["method"]) && $_POST["method"] == "leadAllQuoteIds") {
        $result = leadAllQuoteIds($_POST['lead_id']);
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
    } else if (isset($_POST["method"]) && $_POST["method"] == "singleLeadQuoteDetail") {
        $result = singleLeadQuoteDetail($_POST['quote_id']);
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
        ...
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
    } else if (isset($_POST["method"]) && $_POST["method"] == "leadCountUpdate") {
        $result = leadCountUpdate($_POST['lead_id'], $_POST['count']);
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
...
function fetchVndVechiles($vndid)
{
    ...
    while ($vechileid = $db->fetchByAssoc($sql_results2)) {
        $sql3 = "SELECT name,images,interior_style,base_hourly_rate FROM vnd_vechiles where id ='" . $vechileid['vnd_vechiles_vnd_vendorsvnd_vechiles_idb'] . "' AND deleted=0";
        ...
        while ($vechile = $db->fetchByAssoc($sql_results3)) {
            ...
            $vehicles_object[$vechileid['vnd_vechiles_vnd_vendorsvnd_vechiles_idb']]['base_hourly_rate'] = $vechile['base_hourly_rate'];
            // $vehicles_object['base_hourly_rate'] = $vendor['base_hourly_rate'];
        }
    }
...
function fetchVndLeads($vendor_email, $vendor_id)
{
    ...
    while ($lead = $db->fetchByAssoc($sql_results)) {
        if (!empty($lead['opertunityid_c'])) {
            ...
            $result[$count]['status'] = $lead['status'];
...
function leadCountUpdate($lead_id, $count)
{
    global $db;
    $result = array();
    ...
    // Prepare the SQL statement for insertion
    //  $sql = "UPDATE lead_cstm (q_id , vendor_id) VALUES (".$id." , '".$name."')";
    // $sql = "SELECT * FROM leads_cstm WHERE id_c = '".$lead_id."'";
    $sql = "UPDATE leads_cstm SET sent_lead_count=" . $count . " WHERE id_c='" . $lead_id . "'";
...
function addNewQuote($vendor_id, $lead_id, $quoted_c, $vehicle_c, $vendor_notes_c)
{
    ...
    // Debugging: Echo input parameters to verify values
    // echo "Vendor ID:". $vendor_id.", Lead ID: ".$lead_id.", Quoted Price: ".$quoted_c.", Vehicle: ".$vehicle_c.", Vendor Notes: ".$vendor_notes_c."";

    // Get lead object
    $lead = BeanFactory::getBean('Leads', $lead_id);

    // Debugging: Echo lead object to verify if it's valid
    // var_dump($lead);
    ...
    // Debugging: Echo vendor object to verify if it's valid
    // var_dump($vendor);
    ...
    // Create a new vendor quote bean
    ...
    $bean->quoted_price_c = sprintf('%0.2f', $quoted_c);
    $bean->vehicle_c = $vehicle_c;
    $bean->quote_status_c = "Pending";
    $bean->vendor_notes_c = $vendor_notes_c;
    ...
    if ($lead->status != 'Converted') {
        ...
        echo "L1";
        echo "<br/>";
        // 	echo $lead->load_relationship('leads_tnv_vendorquote_1');
        ...
        if ($lead->load_relationship('leads_tnv_vendorquote_1')) {
            ...
            echo "C1";
            echo "<br/>";
            echo $lead->vnd_vendors_id_c;
            echo "<br/>";
            echo $vendor_quotes_count;
            echo "<br/>";
            if (!empty($lead->vnd_vendors_id_c) && $vendor_quotes_count >= 3) {
                echo "C2";
...
function leadAllQuoteIds($lead_id)
{
    global $db;
    $result = array();

    // Fetch data from leads_tnv_vendorquote_1_c table
    $sql = "SELECT * FROM `leads_tnv_vendorquote_1_c` WHERE `leads_tnv_vendorquote_1leads_ida` = '" . $lead_id . "'";
    $sql_results = $db->query($sql);

    if ($sql_results) {
        $rows = array(); // Array to store all fetched rows
        while ($row = $db->fetchByAssoc($sql_results)) {
            $rows[] = $row;
        }
        if (!empty($rows)) {
            ...
            $result['success'] = "No records found";
        }
    } else {
...
function singleLeadQuoteDetail($quote_id)
{ {
        global $db;
        $result = array();

        // Fetch data from leads_tnv_vendorquote_1_c table
        $sql = "SELECT * FROM `tnv_vendorquote_cstm` WHERE `id_c` LIKE  '" . $quote_id . "'";
        $sql_results = $db->query($sql);
        $sql2 = "SELECT * FROM `tnv_vendorquote` WHERE `id` LIKE '" . $quote_id . "'";
        $sql_results2 = $db->query($sql2);

        if ($sql_results && $sql_results2) {
            $row = $db->fetchByAssoc($sql_results);
            $row2 = $db->fetchByAssoc($sql_results2);
            $result['success'] = "Success";
            $result['data'] = $row;
            $result['data2'] = $row2;
        } else {
...
function fetchAllVendors($vndid)
{

    global $db;
    $result = array();
    $sql = "SELECT * FROM `vnd_vendors` WHERE `id` LIKE '8eee462b-5a6b-5237-64ef-5e66ddee6022'";
    $sql_results = $db->query($sql);
    if ($sql_results) {
