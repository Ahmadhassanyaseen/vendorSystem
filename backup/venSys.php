...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["method"]) && $_POST["method"] == "CreateVendor") {
        ...
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
    } else if (isset($_POST["method"]) && $_POST["method"] == "checkVendor") {
        $result = checkVendor($_POST['vname'], $_POST['vpass']);
        $result['CURL_RESULT'] = "success";
        echo json_encode($result);
    ...
    // } 
        ...
        if ($vnd->id) {
            // Username and password matched, return them for debugging
            if ($vnd->status == "Email_Verification" || $vnd->status == "Email Verification") {
                echo "Please veryif your email address. Open email and click on verification link to activate your profile.";
            } else if ($vnd->status == "Disabled_Admin") {
                echo "Your account was suspended by the administrator!";
...
function fetchVendors($vname)
{
    ...
    $vendor_object = array();
    if (!empty($vname))
        $sql = "SELECT DISTINCT id,name FROM vnd_vendors where STATUS='active' and deleted=0 and (name LIKE '%" . $vname . "%' OR username LIKE '%" . $vname . "%' OR phone_office LIKE '%" . $vname . "%') and password=" . $vpass . "";
    else
...
function checkVendor($vname, $vpass)
{
    global $db;
    $vendor_object = array();
    if (!empty($vname))
